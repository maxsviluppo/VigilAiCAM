...
@Component({
    selector: 'app-food-conservation-view',
    standalone: true,
    imports: [CommonModule],
    template: `
    <div class="space-y-8 pb-10">
        <!-- Premium Header Banner -->
        <div class="bg-gradient-to-r from-blue-600 via-sky-600 to-cyan-600 p-8 rounded-3xl shadow-xl border border-blue-500/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-center gap-4 mt-2">
                    <p class="text-sky-100 text-sm font-medium ml-1">Monitoraggio temperature e stoccaggio</p>
                    <button (click)="showStandardInfo.set(true)" 
                            class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white text-white hover:text-blue-700 transition-all text-[10px] font-black border border-white/30 shadow-md group">
            ...
            <div class="relative z-10 flex flex-col gap-2">
                ...
                <div class="text-xs text-sky-100 font-medium flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @for (check of checks(); track check.id) {
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-300 group"
                     ...
                     [class.border-blue-100]="!check.checked" [class.border-blue-500]="check.checked"
                     [class.bg-blue-50]="check.checked" [class.shadow-lg]="check.checked" [class.shadow-blue-200/50]="check.checked"
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300"
                             [class.bg-blue-500]="check.checked" [class.text-white]="check.checked" [class.shadow-lg]="check.checked"
                             [class.shadow-blue-200]="check.checked" [class.bg-slate-100]="!check.checked" [class.text-slate-400]="!check.checked">
                            <i class="fa-solid text-2xl" [class.fa-check-circle]="check.checked" [class.fa-temperature-low]="!check.checked"></i>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2">
                                @if (check.checked) {
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500 text-white text-xs font-bold rounded-full">
        ...
        @if (showStandardInfo()) {
            <div class="fixed inset-0 z-[110] flex items-center justify-center p-4">
                ...
                <div class="relative bg-white w-full max-w-md max-h-[90vh] rounded-[40px] shadow-2xl overflow-hidden animate-slide-up border border-slate-100 flex flex-col">
                    <div class="p-8 bg-gradient-to-br from-blue-700 to-sky-900 text-white relative flex-shrink-0">
                        <div class="absolute top-0 right-0 p-6 opacity-10 pointer-events-none -rotate-12 translate-x-2">
                            <i class="fa-solid fa-box-archive text-8xl"></i>
                        </div>
                        <div class="relative z-10">
                            <h3 class="text-2xl font-black mb-1">Conservazione</h3>
                            <p class="text-blue-200 text-[10px] font-black uppercase tracking-[0.2em]">Standard HACCP Pro</p>
                        </div>
                    </div>
                    ...
                    <div class="p-8 pt-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-layer-group text-xs"></i> 01. Stoccaggio
                            </h4>
                            <div class="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 text-xs text-slate-700 font-medium leading-relaxed">
                                Separare rigorosamente alimenti crudi da quelli cotti o pronti al consumo per evitare contaminazioni crociate.
                            </div>
                        </div>

                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-tags text-xs"></i> 02. Etichettatura
                            </h4>
                            <div class="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 text-xs text-slate-700 font-medium leading-relaxed">
                                Ogni prodotto deve essere identificabile con data di produzione/apertura e data di scadenza.
                            </div>
                        </div>

                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-arrow-up-9-1 text-xs"></i> 03. FIFO (First-In, First-Out)
                            </h4>
                            <div class="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 text-[10px] font-bold text-slate-500 italic">
                                Utilizzare sempre i prodotti con scadenza più ravvicinata. Controllare quotidianamente l'integrità delle confezioni.
                            </div>
                        </div>
                    </div>
...
export class FoodConservationViewComponent {
    state = inject(AppStateService);
    showStandardInfo = signal(false);
    moduleId = 'food-conservation';

    checks = signal<CheckItem[]>([
        { id: 'perishable', label: 'ALIMENTI DEPERIBILI RIPORRE ALLA TEMPERATURA POSITIVA OR NEGATIVA', checked: false },

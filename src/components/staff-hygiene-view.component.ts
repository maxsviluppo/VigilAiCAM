        ...
        <div class="bg-gradient-to-r from-emerald-600 via-teal-600 to-green-600 p-8 rounded-3xl shadow-xl border border-emerald-500/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-center gap-4 mt-2">
                    <p class="text-emerald-100 text-sm font-medium ml-1">Verifica igiene e abbigliamento operatori</p>
                    <button (click)="showStandardInfo.set(true)" 
                            class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white text-white hover:text-emerald-700 transition-all text-[10px] font-black border border-white/30 shadow-md group">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @for (check of checks(); track check.id) {
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-300 group"
                     ...
                     [class.border-emerald-100]="!check.checked" [class.border-emerald-500]="check.checked"
                     [class.bg-emerald-50]="check.checked" [class.shadow-lg]="check.checked" [class.shadow-emerald-200/50]="check.checked"
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300"
                             [class.bg-emerald-500]="check.checked" [class.text-white]="check.checked" [class.shadow-lg]="check.checked"
                             [class.shadow-emerald-200]="check.checked" [class.bg-slate-100]="!check.checked" [class.text-slate-400]="!check.checked">
                            <i class="fa-solid text-2xl" [class.fa-check-circle]="check.checked" [class.fa-hand-sparkles]="!check.checked"></i>
                        </div>
        ...
        @if (showStandardInfo()) {
            <div class="fixed inset-0 z-[110] flex items-center justify-center p-4">
                ...
                <div class="relative bg-white w-full max-w-md max-h-[90vh] rounded-[40px] shadow-2xl overflow-hidden animate-slide-up border border-slate-100 flex flex-col">
                    <div class="p-8 bg-gradient-to-br from-emerald-700 to-teal-900 text-white relative flex-shrink-0">
                        ...
                        <div class="relative z-10">
                            <h3 class="text-2xl font-black mb-1">Igiene Personale</h3>
                            <p class="text-emerald-200 text-[10px] font-black uppercase tracking-[0.2em]">Standard HACCP Pro</p>
                        </div>
                    </div>
                    ...
                    <div class="p-8 pt-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-soap text-xs"></i> 01. Lavaggio Mani
                            </h4>
                            <div class="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 text-xs text-slate-700 font-medium leading-relaxed">
                                Obbligatorio all'inizio del turno, dopo ogni pausa, dopo aver toccato rifiuti o alimenti crudi.
                            </div>
                        </div>

                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-shirt text-xs"></i> 02. Divisa
                            </h4>
                            <div class="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 text-xs text-slate-700 font-medium leading-relaxed">
                                Deve essere pulita, di colore chiaro, comprensiva di copricapo e calzature idonee. No gioielli o orologi.
                            </div>
                        </div>

                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-notes-medical text-xs"></i> 03. Stato di Salute
                            </h4>
                            <div class="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 text-[10px] font-bold text-slate-500 italic">
                                Comunicare tempestivamente eventuali sintomi di malattie trasmissibili (diarrea, vomito, infezioni cutanee).
                            </div>
                        </div>
                    </div>
